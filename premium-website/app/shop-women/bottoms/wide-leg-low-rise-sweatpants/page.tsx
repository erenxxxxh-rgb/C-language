...
export default function SweatpantsDetailPage() {
  ...
  const colors = [
    { name: "Heather Grey", hex: "#9CA3AF", active: true },
    { name: "Classic Black", hex: "#171717", active: false },
  ...
  return (
    <main className="flex flex-col min-h-screen bg-[#D4A373] text-neutral-900 pt-28 pb-16 px-6 md:px-12">
      ...
      <div className="max-w-6xl mx-auto w-full mb-8">
        <Link href="/shop-women/bottoms" className="inline-flex items-center gap-2 text-sm uppercase tracking-widest text-neutral-800 hover:text-black transition-colors font-medium">
          <ArrowLeft className="w-4 h-4" /> Back to Shorts & Bottoms
        </Link>
      </div>
        ...
        <div className="flex flex-col gap-8 bg-white/50 backdrop-blur-md p-8 md:p-10 rounded-2xl border border-white/30 shadow-sm sticky top-32">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="text-xs font-medium text-neutral-700">(312 Reviews)</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mb-5 text-xs text-neutral-800 bg-white/40 p-4 rounded-xl border border-white/20 grid grid-cols-2 gap-3 shadow-sm">
              ...
              <div><span className="font-bold text-black">Closure:</span> Drawstring</div>
            ...
            <ul className="text-sm text-neutral-800 space-y-3 list-disc list-inside leading-relaxed font-medium">
              <li><strong>Soft & Lightweight:</strong> The blended material delivers a soft texture, stable draping performance, and nice air permeability for a relaxed wearing experience.</li>
              <li><strong>Flowy Design:</strong> Adopts wide leg cuts to create loose and flowy silhouettes. The low-rise design fits gently with a flexible elastic drawstring waist.</li>
              <li><strong>Handy Storage:</strong> Features two side slant pockets offering convenient storage for your small daily items.</li>
              <li><strong>Highly Versatile:</strong> Pairs perfectly with basic tees, cropped tops, hoodies, or blazers. Match with sneakers or casual boots for rich daily collocations.</li>
              <li><strong>Multiple Occasions:</strong> Suited for street styling, casual outings, home relaxation, travel plans, short hikes, and weekend gatherings.</li>
            </ul>
          </div>
